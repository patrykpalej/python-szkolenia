...
Aby przechować określoną wartość w pamięci komputera należy utworzyć **zmienną**. Zmienna to odniesienie (referencja) między pewną nazwą a wydzielonym w pamięci komputera miejscem, które przechowuje określoną wartość i do którego odwołujemy się poprzez użycie zmiennej. Zmienna może być m.in. typu tekstowego, liczbowego (liczba całkowita lub niecałkowita) czy logicznego. Ponieważ **Python jest językiem programowania typowanym dynamicznie**, nie trzeba deklarować typu zmiennej podczas jej definiowania.
...
# True
```
Zmienna `no_value` jest instancją klasy *NoneType*, do której możemy się odnieść poprzez `type(None)`
...
---
>Pozycje te (indeksy) numerujemy od 0, to znaczy że pierwszy znak stringa ma indeks 0. Stąd możemy go również nazywać "zerowym" znakiem, kolejny "pierwszym" itd. Błędna indeksacja stringów oraz innych sekwencji jest częstym błędem początkujących.
---
...
```
`f` przed otwarciem cudzysłowa (pojedynczego lub podwójnego) oznacza że tworzymy f-stringa, natomiast w nawiasach klamrowych wewnątrz umieszczamy zmienną, która ma znaleźć się wewnątrz stringa. Powyższy kod wyprintuje następujący tekst:
...
Innym zastosowaniem komentarzy, którego również nie powinniśmy nadużywać jest komentowanie fragmentów kodu, którego tymczasowo nie planujemy używać ale nie chcemy go też trwale usuwać. Jest to przydatne na etapie testowania i eksperymentowania, ale na dłuższą metę zakomentowany kod nie powinien znajdować się w repozytorium (czyli "miejscu" gdzie składowany jest kod). 
...
Umieszczenie takiego stringa w kodzie działa podobnie jak komentarz blokowy, z tą różnicą że interpreter traktuje go jak pełnoprawnego stringa. Po prostu nie jest on przypisywany do żadnej zmiennej więc nie wpływa na działanie programu.
