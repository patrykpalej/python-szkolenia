...
---
---
Istnieje cały szereg reguł dotyczących tego jakie argumenty czy zmienne są widoczne w poszczególnych scope'ach.
...
Nadużywanie tego nie jest dobrą praktyką, bo korzystając z parametrów funkcji możemy jawnie przekazywać do niej poszczególne wartości w postaci argumentów. Python pozwala jednak na używanie w funkcji zmiennych, które nie zostały przekazane jako argument.
```python
foo = 1
...
# !!! Próba wywołania funkcji func() spowoduje błąd, ponieważ operacja wyprintowania w ostatniej linii funkcji odwołuje się do zmiennej print równej 3 a nie funkcji wbudowanej print() !!!
```
