...
Jednym z charakterystycznych dla Pythona elementów jest tzw. wyrażenie listotwórcze czyli *list comprehension*. Aby utworzyć listę korzystając w tym celu z pętli `for` zapisując wszystko w jednej linijce  możemy użyć nastepującej notacji. To co znajduje się na lewo od *for* to przepis na pojedynczy element listy.
...
- Aby w jednej linijce utworzyć listę posługując się do tego pętlą możem użyć *list comprehension*
- Elementem *list comprehension* może być również instrukcja warunkowa
