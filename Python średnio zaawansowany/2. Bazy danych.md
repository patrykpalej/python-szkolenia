...
&nbsp;
### 2.4 MongoDB
Oprócz relacyjnych baz danych, istnieją również bazy nierelacyjne. Należy do nich m.in. MongoDB. Jest to baza dokumentowa, która opiera się na kolekcjach dokumentów, czyli obiektów analogicznych do słowników w Pythonie. Operacje na takiej bazie przypominają działania na strukturach typu JSON.
...
Dane w postaci obiektu klasy `pymongo.cursor.Cursor`:
```python
