...
---
Python umożliwia również pisanie instrukcji `if-else` w skrócony sposób:
```python
human_power = 4
zombie_power = 3
...
Kolejnym rozwinięciem instrukcji `if` oraz `if-else` jest `if-elif-else`. Używamy jej wtedy kiedy chcemy wykonać jeden z wielu bloków kodu, z których każdy poprzedzony jest warunkiem logicznym. Aby dany blok kodu się wykonał, odpowiadający mu warunek logiczny musi zostać spełniony. Instrukcja `if-elif-else` składa się z następujących elementów:
*if* - pierwszy warunek logiczny. Jeżeli zostanie spełniony to wykonuje się blok kodu pod nim

*elif* - kolejny warunek. Jeżeli zostanie spełniony to wykonuje się blok kodu pod nim. *elifów* może być dowolnie dużo

*else* - można ale nie trzeba na koniec dodać klauzulę *else* pod którą znajduje się blok kodu, który zostanie wykonany jeżeli żaden z powyższych warunków logicznych nie został spełniony
...
if number_of_humans > 0 and number_of_zombies ＝＝ 0:
    ...
    print("Zawsze prawdziwe, ale wykona się tylko jeśli poprzednie "
...
---
Instrukcje warunkowe mogą być w sobie zagnieżdżone:
```python
...
- Aby wykonać dokładnie jeden spośród wielu bloków kodu do których przypisane są warunki logiczne należy użyć instrukcji `if-elif-(else)`
- Klauzula `if` musi być użyta dokładnie raz, `elif` dowolną liczbę razy a `else` może, ale nie musi być użyte
- Instrukcje warunkowe mogą być w sobie zagnieżdżone
- Kiedy jeden z warunków zostanie spełniony zostaje wykonany kod po nim po czym następuje wyjście z instrukcji warunkowej 
---
---
---
&nbsp;
