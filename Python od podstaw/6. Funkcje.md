...
Jego rezultatem jest obiekt typu `map`, który po zrzutowaniu na listę daje taką listę:
```python
...
#### 6.5.2 `filter`
Aby przefiltrować sekwencję pod kątem spełnienia przez jej elementy danego warunku logicznego można użyć generatora `filter`. Należy w tym celu napisać funkcję, która będzie zwracać wartość logiczną *True/False*:
