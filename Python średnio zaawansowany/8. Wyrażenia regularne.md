...
&nbsp;
### 8.3 Znaki specjalne
Patterny zazwyczaj składają się nie tylko z konkretnych znaków, które chcemy znaleźć. Najczęściej znajdziemy w nich znaki specjalne, pozwalające na tworzenie bardziej ogólnych dopasowań. Przykładowo możemy szukać znaków, które nie są cyfrą, albo takich, które stanowią granicę słowa. 

Istnieje również sposób na określenie ile takich znaków ma się znaleźć, na przykład "jeden lub więcej" albo "dokładnie 4".

Poniżej znajduje się zestawienie znaków specjalnych w wyrażeniach regularnych:

```
.       - Dowolny znak (oprócz nowej linii, czyli \n)
\d      - Cyfra (0-9)
\D      - Znak nie będący cyfrą
\w      - Znak, będący częścią słowa, tzw. word character (a-z, A-Z, 0-9, _)
\W      - Znak nie będący word character
\s      - Znak biały (spacja, tab, nowa linia)
\S      - Znak nie będący jednym z powyższych
...
`You can by it for only 10$`

należy użyć następującego patternu:

`\d+\$`

Zakładamy, że cena to ciąg cyfr (przynajmniej jednej). Wyszukujemy więc cyfry powtórzone jeden lub więcej razy (`\d+`) oraz dosłownie znaku "$" – czyli `\$` (a nie końca stringa).
