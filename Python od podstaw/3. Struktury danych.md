...
&nbsp;
### 3.4 set
Ostatnią z 4 najczęściej używanych struktur danych jest `set`. `set`, czyli po polsku "zbiór" to struktura, która przechowuje zestaw unikalnych wartości w nieuporządkowanej formie. To znaczy że poszczególnym elementom nie przypisujemy indeksów wskazujących na ich pozycję w zbiorze. Wartości znajdujące się w zbiorze nie mogą występować w nim wielokrotnie - albo tam są albo ich nie ma.
...
Jeśli spróbujemy powtórzyć w zbiorze jedną wartość kilkukrotnie, w zbiorze i tak znajdzie się ona tylko raz:
```python
