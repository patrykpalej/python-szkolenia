...
W kolejnym przykładzie definiujemy siłę człowieka a następnie listy wartości siły kilku zombie. Następnie iterując po wszystkich zombie człowiek ocenia czy ma szansę z nimi wygrać - jeśli nie to przechodzimy do kolejnej iteracji (człowiek unika starcia) a jeśli nie to dochodzi do walki:
```python
human_power = 10
