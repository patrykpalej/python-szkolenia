...
Kiedy wyświetlamy obiekt w konsoli albo printujemy go, zawsze widzimy kawałek tekstu, który odzwierciedla zawartość obiektu. Są to tzw. **reprezentacja** oraz **stringifikacja**. Reprezentację widzimy po odniesieniu się do obiektu w konsoli lub Jupyter Notebooku, natomiast stringifikacja pojawia się kiedy wyprintujemy obiekt lub zrzutujemy go na stringa. Takie zachowanie możemy obserwować na każdym typie obiektu. Jednak tworząc własne klasy mamy wpływ na to jak wyglądają reprezentacja oraz stringifikacja.
...
class List:
    ...
    def __repr__(self):
        return f"UserDefinedList({', '.join([str(i) for i in self.values])})"
