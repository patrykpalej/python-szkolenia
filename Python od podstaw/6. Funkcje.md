...
Wiele funkcji nie tylko wykonuje określone instrukcje, ale również *zwraca* jedną lub więcej wartości. Oznacza to, że pewne wartości obecne wewnątrz funkcji mogą zostać przechwycone w trakcie jej wywoływania i użyte już poza funkcją. Poniżej prosty przykład:
...
Aby zwrócić kilka wartości należy podać je po  `return` oddzielając je przecinkiem. Funkcja zwróci wówczas tuplę tych wartości.
```python
