...
Podczas definiowania parametrów funkcji istnieje kilka reguł mówiących o tym jaka powinna być ich kolejność.

1. **Argumenty z wartością domyślną deklarujemy po zwykłych**
...
**Niepoprawna** definicja funkcji:
```python
def calculate_distance_to_point(reference=(0, 0), coordinates):
    x_a, y_a = coordinates
    x_b, y_b = reference
...
2. **\*args i \*\*kwargs deklarujemy na końcu**
```python
